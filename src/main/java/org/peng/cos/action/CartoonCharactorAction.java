...
@Named("cartoon")
@RequestScoped
public class CartoonCharactorAction extends BaseAction implements Serializable
{
	@Inject
	...
	public DataModel getPageCartoonCharactors()
	{
		PagerDataModel pdm=
		new PagerDataModel(constant.getPageSize())
		{
			...
			public PagerModel getPagerModel() {
				PagerModel list=  adminManager.listAllCartoonCharactor(ThreadUtil.getPageOffset(), constant.getPageSize());
				return list;
			}
		};
		return pdm;
	...
	public void addOrUpdateCartoon()
	{
		try
		{
			if(inputCharactor.getId()==0)
			{
				...
				inputCharactor.setCtype(CHARACTOR_TYPE.TYPE_CATOON);
